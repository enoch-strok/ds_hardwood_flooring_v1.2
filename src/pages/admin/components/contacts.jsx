...
const Contacts = (props) => {
  let [contactData, setcontactData] = useState([{status: "loading..."}]);

   
  useEffect(async () => {
    const result = await axios.get(
      'https://ds-hardwood-flooring-server.herokuapp.com/contacts/show_contacts'
      );
    console.log(result.data);
    setcontactData(result.data);
  },[]);
  console.log("start of admin: ", contactData);
  

  return (
    <div className="rounded-lg bg-white overflow-hidden shadow">
      <h1 className="mt-20 mb-5 font-black text-5xl">Contacts</h1>
                  ...
                  {contactData.map((person) => (
                    <tr key={person.email}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <div className="ml-4">
                            <div className="text-sm font-medium text-gray-900">{person.message}</div>
                            {/* <div className="text-sm text-gray-500">{person.lastName}</div> */}
                          </div>
                        </div>
                      </td>
